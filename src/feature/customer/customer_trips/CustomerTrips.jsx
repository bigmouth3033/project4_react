...
import { GetUserReservationRequest, GetUserTripsRequest } from "./api/customerTripApi";
...
export default function CustomerTrips() {
  ...
  const getUserReview = GetUserReviewRequest(currentPageReview - 1, 20, reviewStatus);
  ...
  useEffect(() => {
    if (user.isSuccess) {
      setDate([new Date(user.data.data.createdAt), new Date(now.getFullYear() + 2, 0, 1)]);
    }
  }, [user.isSuccess]);
  ...
  return (
    <>
      ...
      <ContainerStyled>
        <HeaderStyled>
          <div>
            <HeaderButton $active={headerType == "trips"} onClick={() => setHeaderType("trips")}>
              Trips
            </HeaderButton>
            <HeaderButton
            ...
            <HeaderButton $active={headerType == "refund"} onClick={() => setHeaderType("refund")}>
              Refund
            </HeaderButton>
            <HeaderButton $active={headerType == "review"} onClick={() => setHeaderType("review")}>
              Review
            </HeaderButton>
          </div>
          {headerType != "review" && (
                  ...
                  <div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (new Date(now.getFullYear(), now.getMonth(), 1) < start) {
                          alert("ss");
                          setDate([start, new Date(now.getFullYear(), now.getMonth() + 1, 0)]);
                        } else {
                          setDate([
                    ...
                    <button
                      onClick={() => {
                        const start = new Date(user.data.data.createdAt);
                        if (new Date(now.getFullYear(), now.getMonth() - 6, 1) < start) {
                          setDate([start, new Date(now.getFullYear(), now.getMonth() + 1, 0)]);
                        } else {
                          setDate([
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (new Date(now.getFullYear(), 0, 1) < start) {
                          setDate([start, new Date(now.getFullYear() + 1, 0, 0)]);
                        } else {
                          setDate([
        ...
        <BodyStyled>
          {headerType == "trips" && (
            <>
              <FilterHeader>
                ...
                >
                  Checking out (
                  {getUserTripsCount.isSuccess ? getUserTripsCount.data.data.checkoutCount : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  Currently stay-in (
                  {getUserTripsCount.isSuccess ? getUserTripsCount.data.data.stayInCount : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  Upcoming (
                  {getUserTripsCount.isSuccess ? getUserTripsCount.data.data.upcomingCount : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  Pending review (
                  {getUserTripsCount.isSuccess ? getUserTripsCount.data.data.pendingCount : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  History stay-in (
                  {getUserTripsCount.isSuccess ? getUserTripsCount.data.data.historyCount : 0})
                </StatusButtonStyled>
              </FilterHeader>
              ...
              {getUserTrips.isSuccess && getUserTrips.data.data.length == 0 && (
                <EmptyListBooking>
                  <CiSquareInfo />
                  {status == "checkout" && <p>You don’t have any stay-in currently checkout.</p>}
                  {status == "stayin" && <p>You don’t have stay-in today.</p>}
                  {status == "pending" && <p>You don’t have any stay-in pending review.</p>}
                  {status == "history" && <p>You currently don’t have any stay-in history.</p>}
                </EmptyListBooking>
              )}
                ...
                <TripContainerStyled>
                  {getUserTrips.data.data.map((trip, index) => {
                    ...
                    province = dchc.data.find(
                      (dchcProvince) =>
                        dchcProvince.level1_id == trip.property.addressCode.split("_")[0]
                    );
                    district = province.level2s.find(
                      (dchcDistrict) =>
                        dchcDistrict.level2_id == trip.property.addressCode.split("_")[1]
                    );

                    return (
                      <TripStyled key={index}>
                        <div onClick={() => navigate("/property_detail/" + trip.property.id)}>
                          <img src={trip.property.propertyImages[0].imageName} />
                          <BookingOption>
                            <div>
                              <PiDotsThreeCircleFill
                                onClick={(ev) => {
                                  ev.stopPropagation();
                                  setIsOptionDropDown((prev) => {
                                    if (prev == trip.id) {
                                      return null;
                                    } else {
                                      return trip.id;
                        ...
                        <div>
                          {formatDate(trip.checkInDay)} - {formatDate(trip.checkOutDay)}
                        </div>
                        <div>$ {formatDollar(trip.amount)}</div>
                        <div>
                          {status == "pending" && (
                            <button onClick={() => setReview(trip)}>Review</button>
                          )}
                          <button onClick={() => setTripDetail(trip)}>Detail</button>
                        </div>
                      </TripStyled>
                    );
                  })}
          ...
          {headerType == "reserved" && (
            <>
              <FilterHeader>
                ...
                >
                  Pending (
                  {getUserReservedCount.isSuccess ? getUserReservedCount.data.data.pending : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  Host denied (
                  {getUserReservedCount.isSuccess ? getUserReservedCount.data.data.denied : 0})
                </StatusButtonStyled>
                <StatusButtonStyled
                ...
                >
                  User cancel (
                  {getUserReservedCount.isSuccess ? getUserReservedCount.data.data.cancel : 0})
                </StatusButtonStyled>
              </FilterHeader>

              {getUserReservation.isSuccess && getUserReservation.data.data.length == 0 && (
                <EmptyListBooking>
                  <CiSquareInfo />
                  {reservedStatus == "pending" && <p>You don’t have any reservation pending.</p>}
                  {reservedStatus == "denied" && <p>You don’t have any reservation denied.</p>}
                  {reservedStatus == "cancel" && <p>You don’t have any canceled reservation.</p>}
                </EmptyListBooking>
              )}

              {getUserReservation.isSuccess && getUserReservation.data.data.length > 0 && (
                <TripContainerStyled>
                  {getUserReservation.data.data.map((trip, index) => {
                    let province;
                    let district;
                    province = dchc.data.find(
                      (dchcProvince) =>
          ...
          {headerType == "review" && (
            <>
              ...
              {getUserReview.isSuccess && getUserReview.data.data.length == 0 && (
                <EmptyListBooking>
                  <CiSquareInfo />
                      ...
                      return (
                        <CommentContainerStyled key={index}>
                          <div>
                            <Avatar
                              round={15}
                              src={review.booking.property.propertyImages[0].imageName}
                            />
                            <button onClick={() => setTripDetail(review.booking)}>
                              Booking detail
                            </button>
                          </div>
                          <div>
      ...
      {tripDetail && <TripDetail action={() => setTripDetail()} booking={tripDetail} />}
