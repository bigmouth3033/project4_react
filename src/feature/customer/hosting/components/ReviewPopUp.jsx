...
import { RateByHostRequest } from "../api/hostingApi";
...
export default function ReviewPopUp({ action, booking, getHostBooking, getHostBookingCount }) {
  const rateByHost = RateByHostRequest();
  ...
  const onRating = () => {
    ...
    formData.append("totalScore", totalRating);
    ...
    formData.append("toUser", booking.customer.id);

    rateByHost.mutate(formData, {
      onSuccess: (response) => {
  ...
  return (
    <PopUpStyled action={action}>
      <ReviewBodyStyled>
        <div>
          <h2>
            How was {booking.customer.firstName} {booking.customer.lastName} stay at your place
          </h2>
        </div>
        <div>
          <Avatar
          ...
          />
          <p>
            Rate {booking.customer.firstName} {booking.customer.lastName} stay at your place
          </p>
        </div>
        <div>
        ...
        <div>
          <TextArea
            ...
            placeholder="Share your thought about this customer"
          />
        </div>
        <div
          onClick={() => {
        ...
        >
          <h3>Additional rating</h3>
          {isAdditionalRating ? <IoIosArrowDown /> : <IoIosArrowUp />}
        </div>
        {isAdditionalRating && (
          <>
            <div>
