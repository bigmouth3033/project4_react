...
export default function PreferredNameOption({ preferredName }) {
    ...
    const [descriptionAfter, setDescriptionAfter] = useState("This is how your first name will appear to Hosts and guests. Learn more");
    ...
    useEffect(() => {
        ...
        if (isDisplay) {
            ...
            preferredName == null || preferredName === "" ? setDescriptionAfter("Not Provided") : setDescriptionAfter(preferredName);
    ...
    return (
        <Container>
            <PersonalSettingOption title="Preferred name" description={descriptionAfter} edit={editAfter} isDisabled={false} onActionClick={setIsDisplay} />
            {isDisplay && (
                <FormGroup>
                    <FormControl>
                        <FormContainer>
                            <FormTitle>Prefered Name (Optional)</FormTitle>
                            <FormInput value={preferredNameInput} onChange={(e) => handlePreferredNameInput(e.target.value)}></FormInput>
                        </FormContainer>
