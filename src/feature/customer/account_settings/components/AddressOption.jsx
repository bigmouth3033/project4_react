...
export default function AddressOption({ addressProp }) {
    ...
    const handleAddressDetailChange = (event) => {
        ...
        if (value.length > 30) {
            setAddressDetailError({ isError: true, message: "Address Detail must be less than 30 characters)" });
            return;
        }

        const regex = /[!@#$%^&*(),.?":{}|<>]/;
        if (regex.test(value)) {
            setAddressDetailError({ isError: true, message: "Address Detail cannot have special characters" });
            return;
        }
    ...
    return (
        <Container>
            <PersonalSettingOption title="Address" description={addressProp != null && addressProp != "" ? addressProp : "Not provided"} edit="Edit" isDisabled={false} onActionClick={setIsDisplay} />
            {isDisplay && (
                <FormGroup>
                    <FormSelectContainer>
                        ...
                        <FormSelect value={city} onChange={handleCityChange}>
                            {
                                address?.map((item) => (
                                    <FormOption key={item.Id} value={item.Id}>{item.Name}</FormOption>
                                ))
                            }
                    ...
                    <FormControl>
                        <FormContainer>
                            ...
                            <FormInput placeholder='Enter your address' value={addressDetail} onChange={handleAddressDetailChange}></FormInput>
                        </FormContainer>
                        {addressDetailError.isError && (
                            <Error>{addressDetailError?.message}</Error>
                        )
                        }
                    </FormControl>
                    <FormSelectContainer></FormSelectContainer>
